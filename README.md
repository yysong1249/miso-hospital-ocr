...
원본은 `miso-hospital-3tier/`, 수정본(이 저장소)은 `miso-hospital-3tier-secure/`에 각각 보존되어 있어 대조 확인이 가능합니다.
이후 이 저장소에는 **문서 스캔(OCR) 기능이 관리자 전용으로 추가**되었습니다 (12번 항목 참고).
...
> **참고**: 이 제한은 IP 단위입니다. 같은 컴퓨터에서 자동화 테스트(예: 헤드리스 브라우저)와 실제 로그인 시도를 동시에 하면 카운트를 공유해서 서로 영향을 줄 수 있습니다 — 로컬 개발 중 원인 모를 429가 뜬다면 이걸 의심해보세요. (재현 사례가 원본 저장소의 `OCR.md`에 기록되어 있습니다.)
...
## 12. 신규 기능: 문서 스캔 (OCR, 관리자 전용)

취약점 수정과는 별개로, 처방전/진단서/영수증 이미지에서 텍스트를 추출해 환자 기록으로 저장하는 기능을 추가했습니다. 원본 저장소(`miso-hospital-3tier/`)에서 먼저 설계·구현한 뒤, 이 저장소의 기존 보안 패턴(CSRF 토큰, bcrypt, AES 암호화 등)에 맞춰 이식했습니다. 설계 과정에서의 세부 의사결정(문서 종류 분류 도입 경위, 필드 파싱 규칙, 신뢰도 강조 표시를 만들었다가 뺀 이유 등)은 원본 저장소의 `OCR.md`에 기록되어 있습니다.
...
// [보안 강화 #5 CSRF] 다른 상태 변경 POST(board.js)와 동일하게 CSRF 토큰 검증을 첫 게이트로 적용
...
- **환자 조회 라우트 없음**: `board.js`의 IDOR을 소유권 검증으로 막은 것과 같은 맥락으로, 환자가 스캔 문서를 조회할 수 있는 라우트 자체를 아예 만들지 않음 (관리자만 조회 가능)
...
| 12 | (신규 기능) 문서 스캔 OCR | ✅ 추가 | 관리자 전용 role 기반 접근 제어 + CSRF/파일 업로드 하드닝 |
